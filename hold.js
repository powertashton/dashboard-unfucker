...
const main = async function (nonce) {
  ...
  const getUtilities2 = async function () {
    let retries = 0;
...
getUtilities2().then(({ keyToClasses, keyToCss, tr }) => {
  ...
  const unfuck2 = async function () {
    //] const $ = window.jQuery;
    if ($a(keyToCss("headerWrapper")).length) { //initial status checks to determine whether to inject or not
      console.log("no need to unfuck2");
      if (!$a("#__h").length) { $a("#__s").remove() }
      return;
    } else if (isDashboard()
    ...
    } else if (!$a(keyToCss("navigationLinks")).length) {
      console.error("page not loaded, retrying...");
      throw "page not loaded";
    } else { console.log("unfuck2ing dashboard...") };

    const pathname = location.pathname.split("/")[1];
    
    userinfo = await window.tumblr.apiFetch(`/v2/user/info`);
    const blogs = await userinfo.response.user.blogs;
    console.log(blogs);
    let $navigationLinks = $a(keyToCss("navigationLinks"));
    let $content = {};
    let $headerWrapper = $str("<nav>", { class: keyToClasses("headerWrapper").join(" "), id: "__hw" });
    let $header = $str("<header>", { id: "__h" });
    let $logoContainer = $(keyToCss("logoContainer"));
    let $createPost = $(keyToCss("createPost"));
    let $heading = $str(`<div class="${keyToClasses("heading").join(" ")}"><h3>Account</h3></div>`);
    let $likeIcon = $str(`<svg xmlns="http://www.w3.org/2000/svg" height="18" width="20" role="presentation" style="--icon-color-primary: rgba(var(--black), 0.65);"><use href="#managed-icon__like-filled"></use></svg>`);
    let $followingIcon = $str(`<svg xmlns="http://www.w3.org/2000/svg" height="21" width="20" role="presentation" style="--icon-color-primary: rgba(var(--black), 0.65);"><use href="#managed-icon__following"></use></svg>`);
    ...
    document.addEventListener('click', () => { //add popover functionality to account subnav
      if (!$str(`${keyToCss("subNav")}:hover`).length
        && !$subnav.attributes.getNamedItem("hidden")) {
    ...
    if (matchPathname()) { //run on non-permalink pages
      ...
      if (isDashboard()) { //reorder tabs for accounts made after 8 may 2023
        ...
        //   if ($str(keyToCss("timelineOptionsItemWrapper")).first().has("a[href='/dashboard/stuff_for_you']").length ? true : false) {
    ...
    $navSubHeader.classList.add(keyToClasses("heading").join(","));
  ...
  requestAnimationFrame(() => unfuck2());
  window.tumblr.on('navigation', () => requestAnimationFrame(() => 
    window.setTimeout(
      unfuck2().catch((e) => 
        window.setTimeout(unfuck2, 400)
      ),
      400
    )
  ));
